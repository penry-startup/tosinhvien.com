...
class StudentRequest extends BaseRequest
{
    ...
    public function rules()
    {
        ...
        $rules = [
            ...
            'phone'     => !empty($data['phone']) ? 'unique:students,phone' : '',
        ...
        if (!empty($id)) {
            $rules['email']    = 'bail|required|email|max:255|unique:students,email,' . $id;
            $rules['phone']    = !empty($data['phone']) ? 'unique:students,phone,' . $id : '';
