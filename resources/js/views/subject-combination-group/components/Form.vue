  ...
  <div class="form-dialog">
    <div class="form-dialog__inner">
      <el-dialog
        ...
        @close="$emit('close')"
      >
        <el-form ref="formSubjectCombinationGroup" :loading="true" :model="form" :rules="formRules" label-position="top">
          <!-- Name Input -->
          ...
          <el-form-item class="d-flex justify-end m-b--0">
            <el-button
              ...
              @click="store('formSubjectCombinationGroup')"
            >
              {{ $t('button.create') }}
            </el-button>
            <el-button
              ...
              size="small"
              class="text--uppercase"
              @click="update('formSubjectCombinationGroup')"
            >
              {{ $t('button.update') }}
            </el-button>
          </el-form-item>
...
export default {
  name: 'FormSubjectCombinationGroup',
  props: {
    isOpened: {
      type: Boolean,
      default: false,
  ...
  methods: {
    getItem(id) {
      subjectCombinationGroupResource.get(id)
        .then(({ data: { data }}) => {
          this.form = data;
          this.$emit('open');
        })
    ...
    store(form) {
      this.$refs[form].validate(valid => {
        if (valid) {
          this.loading = true;
          this.errorsServer = [];
          subjectCombinationGroupResource.store(this.form)
            .then(_ => {
              this.$message({
                showClose: true,
    ...
    update(form) {
      this.$refs[form].validate(valid => {
        if (valid) {
          this.loading = true;
          this.errorsServer = [];
          subjectCombinationGroupResource.update(this.form, this.targetId)
            .then(_ => {
              this.$message({
                showClose: true,
