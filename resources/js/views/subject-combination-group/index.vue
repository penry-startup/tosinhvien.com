<template>
  <div class="page-target">
    <table-panel>
      <template slot="title">
        <small class="text--uppercase">{{ $t('table.title.subjectCombinationGroup') }}</small>
      </template>

      <template slot="tools">
        <el-button type="primary" size="mini" class="text--uppercase" @click="onOpenForm">
          {{ $t('action.add', { model: $t('model.subjectCombinationGroup') }) }}
        </el-button>
      </template>
      ...
      <template slot="table">
        ...
        >
          ...
          <el-table-column :label="$t('table.common.name')" prop="name" sortable="custom" width="250">
            <template slot-scope="{ row }">
              <div class="heading">{{ row.name }}</div>
            </template>
          </el-table-column>

          <el-table-column :label="$t('table.common.description')" prop="description" sortable="custom" width="850">
            <template slot-scope="{ row }">
              <div class="heading">{{ row.description }}</div>
            </template>
          </el-table-column>
    ...
    <form-subject-combination-group
      v-if="dialogVisible"
...
import FormSubjectCombinationGroup from './components/Form';
...
export default {
  name: 'SubjectCombinationGroupIndex',
  components: {
    TablePanel,
    Pagination,
    FormSubjectCombinationGroup,
  },
  layout: 'admin',
  middleware: 'auth',
  data: () => ({
  ...
  methods: {
    async getList() {
      try {
        ...
        const { data } = await subjectCombinationGroupResource.list(this.table.listQuery);
    ...
    onDestroy(id) {
      this.$confirm(this.$t('confirms.permanently_delete.singular', {
        ...
        try {
          this.table.loading = true;
          await subjectCombinationGroupResource.destroy(id);
