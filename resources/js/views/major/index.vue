<template>
  <div class="page-target">
    <table-panel>
      <template slot="title">
        <small class="text--uppercase">{{ $t('table.title.major') }}</small>
      </template>

      <template slot="tools">
        <el-button type="primary" size="mini" class="text--uppercase" @click="onOpenForm">
          {{ $t('action.add', { model: $t('model.major') }) }}
        </el-button>
      </template>
      ...
      <template slot="table">
        ...
        >
          ...
          <el-table-column :label="$t('table.common.code')" prop="code" sortable="custom" width="150">
            <template slot-scope="{ row }">
          ...
          <el-table-column :label="$t('table.common.thpt_score')" width="120">
            <template slot-scope="{ row }">
              <div class="heading">{{ row.thpt_score || '#N/A' }}</div>
            </template>
          </el-table-column>

          <el-table-column :label="$t('table.common.hocba_score')" width="120">
            <template slot-scope="{ row }">
              <div class="heading">{{ row.hocba_score || '#N/A' }}</div>
            </template>
          </el-table-column>

          <el-table-column :label="$t('table.common.dgnl_score')" width="120">
            <template slot-scope="{ row }">
              <div class="heading">{{ row.dgnl_score || '#N/A' }}</div>
            </template>
          </el-table-column>

          <el-table-column :label="$t('table.common.university')" width="250">
            <template slot-scope="{ row }">
              <div class="heading">{{ row.university && row.university.name }}</div>
            </template>
          </el-table-column>
...
import FormMajor from './components/Form';
  ...
  methods: {
    ...
    onDestroy(id) {
      this.$confirm(this.$t('confirms.permanently_delete.singular', {
        ...
        try {
          this.table.loading = true;
          await majorResource.destroy(id);
