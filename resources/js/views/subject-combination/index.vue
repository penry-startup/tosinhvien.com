<template>
  <div class="page-target">
    <table-panel>
      <template slot="title">
        <small class="text--uppercase">{{ $t('table.title.subjectCombination') }}</small>
      </template>

      <template slot="tools">
        <el-button type="primary" size="mini" class="text--uppercase" @click="onOpenForm">
          {{ $t('action.add', { model: $t('model.subjectCombination') }) }}
        </el-button>
      </template>
      ...
      <template slot="table">
        ...
        >
          ...
          <el-table-column :label="$t('table.common.group')" width="250">
            <template slot-scope="{ row }">
              <router-link to="/">{{ row.group && row.group.name }}</router-link>
            </template>
          </el-table-column>
...
import FormSubjectCombination from './components/Form';
  ...
  methods: {
    async getList() {
      try {
        ...
        const { data } = await subjectCombinationResource.list(this.table.listQuery);
    ...
    onDestroy(id) {
      this.$confirm(this.$t('confirms.permanently_delete.singular', {
        ...
        try {
          this.table.loading = true;
          await subjectCombinationResource.destroy(id);
