<template>
  <div class="page-target">
    <table-panel>
      <template slot="title">
        <small class="text--uppercase">{{ $t('table.title.university') }}</small>
      </template>

      <template slot="tools">
        <el-button type="primary" size="mini" class="text--uppercase" @click="onOpenForm">
          {{ $t('action.add', { model: $t('model.university') }) }}
        </el-button>
      </template>
      ...
      <template slot="table">
        ...
        >
                    ...
                    <el-tab-pane label="Detail" name="detail">
                      ...
                      <div class="inner">
                        <el-descriptions :column="1" border>
                          <el-descriptions-item :label="$t('form.field.phone')">
                            <a v-if="row.phone" :href="`tel:${row.phone}`">{{ row.phone }}</a>
                            <span v-else>#N/A</span>
                          </el-descriptions-item>
                          <el-descriptions-item :label="$t('form.field.website')">
                            <a v-if="row.website" :href="row.website" target="_blank">{{ row.website }}</a>
                            <span v-else>#N/A</span>
                          </el-descriptions-item>
                          <el-descriptions-item :label="$t('form.field.address')">
                            <a v-if="row.address" :href="`https://www.google.com/maps/place/${row.address}`" target="_blank">{{ row.address }}</a>
                            <span v-else>#N/A</span>
                          </el-descriptions-item>
                        </el-descriptions>
                      </div>
          ...
          <el-table-column :label="$t('table.common.name')" prop="name" sortable="custom" width="400">
            <template slot-scope="{ row }">
              <div class="heading">{{ row.name }}</div>
            </template>
          </el-table-column>

          <el-table-column :label="$t('table.common.code')" prop="code" sortable="custom" width="300">
            <template slot-scope="{ row }">
              <div class="heading">{{ row.code }}</div>
            </template>
          </el-table-column>

          <el-table-column :label="$t('table.common.type')" width="200">
            <template slot-scope="{ row }">
              <div class="heading">{{ univTypeList.label[row.type] }}</div>
            </template>
          </el-table-column>

          <el-table-column :label="$t('table.common.city')" width="200">
            <template slot-scope="{ row }">
              <div class="heading">{{ row.city && row.city.name }}</div>
            </template>
          </el-table-column>

          <el-table-column :label="$t('table.common.majors_count')" width="200">
            <template slot-scope="{ row }">
              <div class="heading">{{ row.majors_count }}</div>
            </template>
          </el-table-column>
...
import FormUniversity from './components/Form';
  ...
  methods: {
    async getList() {
      try {
        ...
        const { data } = await universityResource.list(this.table.listQuery);
    ...
    onDestroy(id) {
      this.$confirm(this.$t('confirms.permanently_delete.singular', {
        ...
        try {
          this.table.loading = true;
          await universityResource.destroy(id);
