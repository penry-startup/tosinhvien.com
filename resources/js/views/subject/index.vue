<template>
  <div class="page-target">
    <table-panel>
      <template slot="title">
        <small class="text--uppercase">{{ $t('table.title.subject') }}</small>
      </template>

      <template slot="tools">
        <el-button type="primary" size="mini" class="text--uppercase" @click="onOpenForm">
          {{ $t('action.add', { model: $t('model.subject') }) }}
        </el-button>
      </template>
      ...
      <template slot="table">
        ...
        >
          ...
          <el-table-column :label="$t('table.common.code')" prop="code" sortable="custom" width="350">
            <template slot-scope="{ row }">
  ...
  methods: {
    async getList() {
      try {
        ...
        const { data } = await subjectResource.list(this.table.listQuery);
    ...
    onDestroy(id) {
      this.$confirm(this.$t('confirms.permanently_delete.singular', {
        ...
        try {
          this.table.loading = true;
          await subjectResource.destroy(id);
